...
class TradingBotService {
    ...
    private async simulateTrading(walletAddress: string, config: TradingBotConfig) {
        ...
        const ethBalance = parseFloat(this.web3Instance.utils.fromWei(balance, 'ether'));

        // Simulate price movement
        const priceChange = (Math.random() * 2 - 1) * 0.002; // Random price change ±0.2%
        const newPrice = stats.lastPrice ? stats.lastPrice * (1 + priceChange) : ethBalance;
        ...
        // Update stats
        const profitChange = stats.currentPosition === 'long' ? priceChange : -priceChange;
        ...
        if (stats.totalTrades > 0) {
            ...
            stats.winRate = ((stats.winRate * (stats.totalTrades - 1)) + (successRate > 0.5 ? 1 : 0)) / stats.totalTrades;
        }
