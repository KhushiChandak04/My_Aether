...
export class AITradingBot extends EventEmitter {
  ...
  private setupEventListeners(): void {
    ...
    this.on("tradingSignal", async (signal: TradingSignal) => {
      ...
      try {
        ...
        if (signal.confidence < this.minConfidence) {
          console.log(
            `Signal confidence ${signal.confidence} below threshold ${this.minConfidence}, skipping trade`
          );
          return;
        }
        ...
        const totalCost = BigInt(
          Math.floor(signal.amount * signal.price * 100_000_000)
        );

        if (signal.action === "BUY" && balance < totalCost) {
          throw new Error(
            `Insufficient balance for trade. Required: ${totalCost.toString()} Octas, Available: ${balance.toString()} Octas`
          );
        }
    ...
    this.on(
      "tradeExecuted",
      (data: { signal: TradingSignal; transaction: Types.UserTransaction }) => {
        ...
        console.log(
          "Transaction:",
          `https://explorer.aptoslabs.com/txn/${data.transaction.hash}?network=devnet`
        );
      }
    );
    ...
    while (this._isRunning) {
      ...
      if (now - this.lastCheck >= this.checkInterval) {
        try {
          ...
          const [name, parameters] = await this.ai.getStrategyDetails(
            this.strategyId
          );
          ...
          if (signal) {
            ...
            const payload = {
              ...
              arguments: [
                ...
                Math.floor(signal.amount * 100_000_000), // Convert to Octas
                Math.floor(signal.price * 100_000_000), // Convert to Octas
              ],
            };
